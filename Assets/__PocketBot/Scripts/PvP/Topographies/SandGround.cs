...
public class SandGround : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _sandEffectPrefab;
    ...
    private void OnTriggerStayCallback(Collider collider)
    {
        ...
        if (chassis.CarPhysics.CurrentRaycastHitTarget.colliderInstanceID != default)
        {
            ...
            {
                EnterSandGround(chassis);
            }
            else
            {
                ExitSandGround(chassis);
            }
        }
    }
    ...
    private void AddVFX(PBChassis pBChassis)
    {
        for (int i = 0; i < pBChassis.PartContainers.Count; i++)
        {
                ...
                pBChassis.PartContainers[i].PartSlotType == PBPartSlot.Wheels_3)
            {
                ...
                {
                    var sandVFX = Instantiate(_sandEffectPrefab, pBChassis.transform);
                    sandVFX.transform.SetParent(wheels[x]);
                    sandVFX.transform.localPosition = new Vector3(0, -0.2f, 0.2f);

                    SandEffect sandScript = sandVFX.GetOrAddComponent<SandEffect>();
    ...
    {
        var listVFX = pBChassis.GetComponentsInChildren<SandEffect>();
    ...
    private void EnterSandGround(PBChassis chassis)
    {
    ...
    private void ExitSandGround(PBChassis chassis)
    {
        if (!robotEnterStateDict.Get(chassis.Robot))
            return;
        chassis.CarPhysics.CarTopSpeedMultiplier /= 0.5f;
